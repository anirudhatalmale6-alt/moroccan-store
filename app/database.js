...
if (productCount.count === 0) {
  ...
  if (product) {
    ...
    const insertReview = db.prepare('INSERT INTO reviews (product_id, name, rating, message, status) VALUES (?, ?, ?, ?, ?)');
    seedReviews.forEach(r => insertReview.run(product.id, r.name, r.rating, r.message, 'approved'));
  }
